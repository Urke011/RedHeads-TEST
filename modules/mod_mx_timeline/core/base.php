...
JLoader::register('ImageHelper', dirname(__FILE__).'/image.php');
...
if (!class_exists('BaseHelper')){
	...
	abstract class BaseHelper{
		...
		*/
		 ...
		 */
		public static function getAImage($item, $params, $prefix='imgol'){
		 ...
		 * @param unknown_type $params
		 */
		public static function getAImages($item, $params, $prefix='imgol'){
			...
			if ( !isset(self::$image_cache[$hash][$item->id]) ){
				...
				if ( count($images_path) == 0){
					$images_path[] = array('src'=> $params->get($prefix.'_placeholder_path', 'modules/mod_mx_timeline/assets/images/noimage.jpg'), 'class'=>'placeholder');
				}
		...
		public static function getCImage($item, $params, $prefix='imgol'){
			$images = &self::getCImages($item, $params, $prefix);
			return is_array($images) && count($images) ? $images[0] : null;
		}

		public static function getCImages($item, $params, $prefix='imgol'){
