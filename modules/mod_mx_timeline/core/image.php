...
class ImageHelper{
	...
	public static function setDefault($params = null){
		...
		if ($params instanceof JRegistry){
			...
			// output type ?
			$imgol_type = $params->get('imgol_type', 0);
			if (in_array($imgol_type, array(IMAGETYPE_JPEG, IMAGETYPE_PNG, IMAGETYPE_GIF))){
				$opts['type'] = $imgol_type;
			}

			// function ?
			$imgol_fn = $params->get('imgol_function', 'none_resize');
			$imgol_fn = explode('_', $imgol_fn);
			if ( in_array($imgol_fn[0], self::$functions) ){
				$opts['function'] = $imgol_fn[0];
				if ($imgol_fn[0] == 'resize'){
					if ( isset($imgol_fn[1]) && in_array($imgol_fn[1], self::$modes) ){
						$opts['function_mode'] = $imgol_fn[1];
					} else {
			...
			// background
			$imgol_background = $params->get('imgol_background', null);
			if (!is_null($imgol_background)){
				$opts['background'] = $imgol_background;
			}

			// quality
			$imgol_quality = $params->get('imgol_quality', null);
			if ( $imgol_quality != null && intval($imgol_quality) > 0 && intval($imgol_quality) < 100 ){
				$opts['quality'] = intval($imgol_quality);
			}

			// transparency
			$imgol_transparency = $params->get('imgol_transparency', 1);
			if ( $imgol_transparency != null){
				$opts['transparency'] = $imgol_transparency ? true : false;
			}

			// dimension ?
			$imgol_w = $params->get('imgol_width', 0);
			$imgol_h = $params->get('imgol_height', 0);
			if ( intval($imgol_w) > 0 ){
				$opts['width'] = $imgol_w;
			}
			if ( intval($imgol_h) > 0 ){
			...
			self::$cache = $params->get('imgol_cache');
			self::$cache_url = $params->get('imgol_cache_url');
