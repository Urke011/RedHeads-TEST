...
$document->addStyleSheet($modPath.'assets/css/style2.css');

if($bgm_color) $document->addStyleDeclaration('#tmln' . $sliderid . ' .main-timeline .timeline-content:hover,#tmln' . $sliderid . ' .main-timeline .timeline:hover:after,.main-timeline .timeline-content .btn-link{ background-color: ' . $bgm_color . ';} .main-timeline .year,#tmln' . $sliderid . ' .main-timeline .title { color: ' . $bgm_color . ';} #tmln' . $sliderid . ' .main-timeline .timeline-content:hover:after {border-top-color: ' . $bgm_color . ';} '); 
...
<span class="post"><a href="<?php echo $item->link ?>" title="<?php echo $item->title; ?>" <?php echo MxTimeLineHelper::parseTarget($params->get('link_target')); ?>>
